document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchData() {
        try {
            const response = await fetch('https://lab6-back-kyebr9cv9-fazerits-projects.vercel.app/api');
            if (!response.ok) {
    ...
    async function deleteText(id) {
        try {
            const response = await fetch(`https://lab6-back-kyebr9cv9-fazerits-projects.vercel.app/api/delete/${id}`, {
                method: 'DELETE',
                credentials: 'include'
            });
            if (!response.ok) {
